...
export function createOrderLookupActions(page: Page) {
  const orderInput = page.getByRole('textbox', { name: 'Número do Pedido' }) // or getByLabel('Número do Pedido')
  ...
  return {
    ...
    async validateStatusBadge(status: OrderStatus) {
      ...
      await expect(statusBadge).toHaveClass(new RegExp(statusClasses[status].background))
      await expect(statusBadge).toHaveClass(new RegExp(statusClasses[status].text))
      await expect(statusBadge.locator('svg')).toHaveClass(new RegExp(statusClasses[status].icon))
    },
